...
base_pheno_path = 'yourpath_phe.csv'

# -------------------- Variant Configuration --------------------
variant_name = 'Wide_kernel_PCA'   # You may change this
dilations = [1]                    # dilation = 1
kernel_size = 7                     # kernel = 7
...
# -------------------- Define DCF_VAE with Stability Fixes --------------------
class DCF_VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        # FIX 1: Clamp logvar to prevent extreme values that cause NaN in exp
...
class SNPModel(nn.Module):
    ...
    def forward(self, x):
        z = self.vae(x)
        # FIX 2: Optional layer norm to stabilize latent before graph
...
# -------------------- Loop Over All Traits with Early Stopping --------------------
...
for trait in traits:
    ...
    for fold, (train_idx, val_idx) in enumerate(kf_outer.split(X_tensor)):
        ...
        # Input dimension is now pca_components
        vae = DCF_VAE(input_dim=pca_components, latent_dim=64,
        ...
        for epoch in range(100):
            ...
            with torch.no_grad():
                for batch_X, batch_y in val_loader:
                    ...
                    val_loss += F.mse_loss(y_pred, batch_y, reduction='sum').item()
        ...
        epoch_time = (time.time() - start_time) / (epoch+1)  # average time per epoch
        ...
        if np.any(np.isnan(preds)) or np.any(np.isinf(preds)) or np.any(np.isnan(truths)) or np.any(np.isinf(truths)):
            print(f"Warning: NaNs or infs detected in predictions/truths for fold {fold+1}. Setting all metrics to NaN.")
            corr = np.nan
            mse = np.nan
            recon_mse = np.nan
        else:
    ...
    summary = pd.DataFrame([{
        'trait': trait,
        'corr_mean': fold_df['Corr'].mean(skipna=True),
        'corr_std': fold_df['Corr'].std(skipna=True),
        'mse_mean': fold_df['MSE'].mean(skipna=True),
        'recon_mse_mean': fold_df['Recon_MSE'].mean(skipna=True),
        'time_per_epoch': fold_df['Time_per_epoch'].mean()
    }])
    summary.to_csv(os.path.join(variant_dir, f'{trait}_summary.csv'), index=False)

    print(f"    {trait}: Corr = {summary['corr_mean'].values[0]:.4f} ± {summary['corr_std'].values[0]:.4f} (based on {fold_df['Corr'].count()} valid folds)")
