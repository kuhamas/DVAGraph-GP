...
# Process only the 'Prot' trait (file number 6)
traits = ['Prot']
trait_to_file = {
    'tkw': 1,
    'testw': 2,
...
base_pheno_path = 'yourpath'

# -------------------- Variant Configuration --------------------
variant_name = 'Local_only'
dilations = [2]
kernel_size = 3
...
def safe_pearsonr(x, y):
    """
    Compute Pearson correlation safely, handling NaNs, infs, or constant arrays.
    Returns 0.0 if correlation cannot be computed.
    """
    x = np.asarray(x)
    y = np.asarray(y)
    # Remove any NaNs or infs (should not happen, but just in case)
...
for trait in traits:
    ...
    # Basic check for NaNs in phenotype
    if np.any(np.isnan(y)):
        print(f"Warning: NaNs found in phenotype for {trait}. Removing them.")
        y = y[~np.isnan(y)]
        # Note: This assumes rows correspond; if there are NaNs, you should ensure X is aligned.

    y_tensor = torch.tensor(y, dtype=torch.float32).unsqueeze(1).to(device)

    fold_results = []  # [corr, mse, recon_mse, time_per_epoch] for each fold
    ...
    for fold, (train_idx, val_idx) in enumerate(kf_outer.split(X_tensor)):
        ...
        # Remove any non-finite values (NaNs or infs)
        finite_mask = np.isfinite(preds) & np.isfinite(truths)
        if not np.all(finite_mask):
            n_invalid = np.sum(~finite_mask)
            print(f"  Fold {fold+1}: {n_invalid} non-finite values removed from predictions/truths.")
            preds_clean = preds[finite_mask]
            truths_clean = truths[finite_mask]
        else:
            preds_clean = preds
            truths_clean = truths

        # Compute metrics only if we have at least 2 valid samples
        if len(preds_clean) < 2:
            print(f"  Fold {fold+1}: insufficient valid data for metrics. Setting corr=0.0, mse=NaN.")
            corr = 0.0
            mse = np.nan
            recon_mse = np.nan
        else:
            corr = safe_pearsonr(truths_clean, preds_clean)
            mse = mean_squared_error(truths_clean, preds_clean)
            # Check reconstruction errors for non-finite values
            recon_errors = np.array(recon_errors)
            if np.any(~np.isfinite(recon_errors)):
                print(f"  Fold {fold+1}: non-finite values in reconstruction errors, setting recon_mse=NaN.")
                recon_mse = np.nan
            else:
...
print(f"\n✅ Trait '{traits[0]}' done for variant '{variant_name}'. Results saved in {variant_dir}")